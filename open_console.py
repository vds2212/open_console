...
def switch_to_tab(working_dir, command):
    ...
    if ret == "Yes":
        ...
        current_dir, hwnd = run_script('GetInfo("CurDir","HWND");')
        ...
        print("New process")
        subprocess.Popen([CONEMU_GUI, "-Dir", working_dir])
        if command:
            time.sleep(0.2)
            escaped_command = command
            run_script('Print("%s");' % escaped_command)
...
def run_script(script):
    ...
    print(" " * 4 + command)
    process = subprocess.Popen(command, executable=CONEMU_CONSOLE, stdout=subprocess.PIPE)
    process.wait()
    ...
    result = result[-1]
    ret = result.split(b"\n")
    ret = [x.decode(CONSOLE_CODE_PAGE) for x in ret]
    print(" " * 4 + "Result:", ret)
